...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Cosmos Collage — Reconstruct the cosmos from the cosmos" },
      {
        name: "description",
        content:
...
function Landing() {
  const { openDemo, loadingDemo, ready } = useStudio();
  const navigate = useNavigate();
  const [pending, setPending] = useState<null | "demo" | "upload">(null);
  const fileRef = useRef<HTMLInputElement>(null);
  const { addUploads } = useStudio();
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto flex min-h-screen max-w-4xl flex-col justify-center px-6 py-16">
        <p className="label-xs">Cosmos Collage</p>
        ...
        <p className="mt-6 max-w-xl text-sm leading-relaxed text-muted-foreground">
          Cosmos Collage rebuilds an astronomical image out of fragments cut from other real
        ...
        <dl className="mt-16 grid gap-6 border-t border-border pt-8 sm:grid-cols-3">
          {[
            ...
            ["Matching", "Brightness, colour, contrast, edge density and structure — Visual Analysis."],
            ["Physical", "Assembly map, tile manifest and exports for cutting the collage by hand."],
          ].map(([k, v]) => (
            <div key={k}>
